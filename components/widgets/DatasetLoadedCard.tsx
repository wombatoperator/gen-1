  ...
  return (
    ...
    >
      <dl className="grid grid-cols-2 sm:grid-cols-4">
        <Field label="Rows" value={data.rowCount.toLocaleString()} sub={`${data.columnsPresent.length} columns`} />
        <Field
          label="Window"
          value={data.dateRange ? `${data.dateRange.days} days` : '—'}
          sub={data.dateRange ? `${data.dateRange.start} → ${data.dateRange.end}` : 'No date column'}
        />
        <Field label="Mode" value={data.isTimeSeries ? 'Time series' : 'Snapshot'} sub={data.truncated ? 'Truncated' : 'Complete'} />
        <Field
          label="Errors"
  ...
  const valueColor =
    tone === 'positive'
      ...
      : tone === 'warning'
        ...
        : 'text-[var(--color-ink)]';
  return (
    <div className="px-5 py-3.5 border-r border-b border-[var(--color-rule-soft)] last:border-r-0 sm:last-of-type:border-r-0">
